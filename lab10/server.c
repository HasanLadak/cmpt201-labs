...
Its using TCP and uses write() to send data
2. What data is the client sending to the server?
The client is sending 5 strings (Hello, Apple, Car, Green, Dog) and its sent in
a buffer Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
Its passed pointer to a struct which which contains pointer to the shared list,
pounter to the mutex protecting it and a run flag.
2. How are received messages stored?
Messages are stored in a linked list, each message received becomse a
dynamically allocated list_node which contains pointer to the msg data and next
pointer.
3. What does `main()` do with the received messages?
main calles collect_all which goes through the linked list, prints each msg, and
frees all memory.
4. How are threads used in this sample code?
one acceptor thread runs run_acceptor, one client handling thread per connectio,
each clinet thread runs run_client.

Explain the use of non-blocking sockets in this lab.
How are sockets made non-blocking?
sockets are made non-blocking by calling fcntl(fd, F_SETFL, flags | O_NONBLOCK)
What sockets are made non-blocking?
the listnening socket and each clinet socket qre set to non blocking
Why are these sockets made non-blocking? What purpose does it serve?
so acceot and read return immediately instead of freezing the thread
*/
